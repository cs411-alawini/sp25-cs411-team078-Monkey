...
app.get('/api/study-sessions', async (req, res) => {
  try {
    ...
    if (req.query.course) {
      const sessions = await sessionQueries.getSessionsByCourse(req.query.course);
...
app.post('/api/study-sessions', async (req, res) => {
  try {
    ...
    if (req.body.creatorNetId) {
      await userQueries.updateUserSession(req.body.creatorNetId, sessionResult.insertId);
    }
    
    // Get the newly created session with full details
    const newSession = await sessionQueries.getSessionById(sessionResult.insertId);
